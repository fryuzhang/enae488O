...
void do_something() {
    uint32_t deltat = kilo_ticks - start
	uint32_t offset = (uint32_t)(max_seen - my_rank) * DELAY_X;
	if (deltat < offset) {
        set_color(RGB(0, 0, 0));
        return;
    }
    uint32_t t = deltat - offset;
    if (t < (uint32_t)STEP_DELAY) {
       move();
   } else {
	 show_rank_color();
