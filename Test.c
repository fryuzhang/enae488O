...
void rebuild_known_uids(void) {
    uint8_t i;
    known_uid_count = 0;
    add_uid_to_known(my_uid);
    for (i = 0; i < neighbor_count; i++)
        add_uid_to_known(neighbors[i].uid);
}
...
void check_if_line(void) {
    ...
    max_nc = my_neighbor_count;
    for (i = 0; i < neighbor_count; i++)
        if (neighbors[i].neighbor_count > max_nc)
            max_nc = neighbors[i].neighbor_count;
    if (max_nc >= 3) return;
    mode = LINE_MODE;
    flood_counter = 0;
}
...
message_t *message_tx(void) { return &tx_msg; }
void message_tx_success(void) { msg_sent = 1; }
...
void loop(void) {
    uint8_t graph_size, i, uid_to_send;
    ...
    if (mode == GRAPH_MODE) {
        ...
        if (my_neighbor_count >= 3) {
            ...
            if (my_neighbor_count == 0 && known_uid_count == 1) {
                set_color(RGB(0,0,3));
                mode = ISOLATED;
            } else {
                mode = FLASHING;
